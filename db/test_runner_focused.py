# -*- coding: utf-8 -*-
"""100-question runner. Reads model id from env, prompt from /tmp/live_prompt.txt,
all 40 questions hardcoded. Writes /tmp/test_results_focused.json"""
...
# load the 40 questions written to /tmp/questions_focused.py
import importlib.util
spec = importlib.util.spec_from_file_location("q", "/tmp/questions_focused.py")
...
if __name__ == "__main__":
    ...
    for i, q in enumerate(QUESTIONS, 1):
        r = run_one(i, q)
        results.append(r)
        # crash-safe: persist after every question
        open("/tmp/test_results_focused.json","w",encoding="utf-8").write(
    ...
    open("/tmp/test_results_focused.json","w",encoding="utf-8").write(
        json.dumps(results, ensure_ascii=False, indent=2))
    print("\nfull log saved to /tmp/test_results_focused.json")
