...
POLICY = (
    ...
    "1. Имена показателей сокращены («груз.», «в т.ч.»). Для поиска используй "
    "`ORDER BY similarity(name, 'смысл') DESC LIMIT 10` — НЕ через `name % '...'` "
    "(порог 0.3 часто отбрасывает короткие термы и даёт 0 строк). "
    "После получения списка кандидатов — выбери подходящий и запрашивай данные, не уточняй поиск дальше.\n"
