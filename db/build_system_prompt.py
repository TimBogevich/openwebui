...
РАЗБОР ПОКАЗАТЕЛЯ
На запрос «проанализируй / представь результаты / причины / конкретные цифры» — сверься с образцом: search_knowledge('образец доклада', collection='reference'). Приводи факт и отклонения по всем трём периодам (сутки, с начала месяца, с начала года): факт %, отклонения от плана и к прошлому году в п.п. Зона — словом, со ссылкой на источник.
