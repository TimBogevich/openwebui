...
COMMENT ON TABLE spravki_port_stations IS
  ...
  'СТАНЦИИ с «неэффективным использованием / наибольшим резервом» (вопрос про станции, row_level=''port'') — '
  'ранжируй по АБСОЛЮТНОМУ неиспользованному резерву (capacity − load_fact) DESC, а НЕ по минимальному '
  'коэффициенту: мелкие терминалы с мощностью в десятки вагонов дают околонулевой %, но их абсолютный '
  'резерв ничтожен — это не показатель неэффективности сети; значимый резерв дают только крупные терминалы. '
...
COMMENT ON TABLE spravki_speed IS
  ...
  'Основные колонки для ответа: norm, day_fact, day_delta, month_fact, month_delta, year_delta — '
  'это ГРУЗОВОЕ движение (общий показатель). '
  'Колонки nopass_* и pass_* — это АЛЬТЕРНАТИВНЫЕ методические разрезы той же справки '
  '(«без учёта пассажирского движения» и «пассажирское движение» соответственно). '
  'Они НЕ являются причинами/факторами невыполнения скорости и НЕ должны использоваться для '
  'объяснения причин отклонения; разность day_fact − nopass_day_fact НЕ означает «влияние '
  'пассажирского движения». Для вопроса «причины невыполнения участковой скорости» используй '
  'ТОЛЬКО day_delta/month_delta (отклонение от нормы) по дорогам + данные об ограничениях '
  'скорости (spravki_speed_restrictions) и отказах (spravki_failures). '
  'Источник справок к ГЦУ. Связывается по report_date.';
