...
COMMENT ON TABLE spravki_delays IS
  ...
  'ВНИМАНИЕ ПРИ АГРЕГАЦИИ: таблица содержит И детальные строки по дорогам, И уже '
  'посчитанные итоги. Итог по сети = строка road_code=''СЕТЬ'' (НЕ суммируй дороги — '
  'удвоишь). Итог по всем кодам = строка delay_code=''ВСЕГО''. Для «сколько всего по сети» '
  'бери готовую итоговую строку, для разреза по дорогам — фильтруй конкретную дорогу.';
